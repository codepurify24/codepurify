...
def calculate_important_score(code, tokenizer, victim_model, mlm_model, max_length):
    ...
    with torch.no_grad():
        ...
        preds = victim_model.generate(ori_input_ids,
                                      attention_mask=ori_mask_ids,
                                      use_cache=True,
                                      num_beams=1,
                                      max_length=max_length)
        top_preds = list(preds.cpu().numpy())
        target_code = tokenizer.decode(top_preds[0], skip_special_tokens=True, clean_up_tokenization_spaces=False)
    ...
    mlm_model.train()
    with torch.no_grad():
        logits = victim_model(input_ids=all_unk_input_ids, attention_mask=all_unk_mask_ids,
                                         labels=target_input_ids, decoder_attention_mask=target_mask_ids).logits
        loss_fct = nn.CrossEntropyLoss(ignore_index=-100, reduction='none')
        loss = loss_fct(logits.view(-1, logits.size(-1)), target_input_ids.view(-1))
        loss = loss.view(target_input_ids.size(0), -1)
        importance_scores = torch.mean(loss, dim=-1)
...
def predict_mask(input_ids, mask_position, mlm_model, mlm_tokenizer, victim_tokenizer, purification_num, variable_scores=None):
    ...
    purified_input_ids = victim_tokenizer.encode(decoded_text, max_length=max_length, padding='max_length',
                                                 truncation=True, )
    purified_input_ids = torch.tensor([purified_input_ids]).cuda()
    return purified_input_ids, purified_input_ids.ne(victim_tokenizer.pad_token_id).to(torch.int64).cuda()
...
def asr_defensive_inference(file, benign_model_file, victim_model_file, mlm_model_file, max_length, purification_num=10):
    ...
    for line in tqdm(data_lines[:]):
        ...
        with torch.no_grad():
            preds = victim_model.generate(purified_ids,
                                          attention_mask=purified_mask,
                                          use_cache=True,
                                          num_beams=1,
                                          max_length=max_length)
            top_preds = list(preds.cpu().numpy())
            ref = victim_tokenizer.decode(top_preds[0], skip_special_tokens=True, clean_up_tokenization_spaces=False)
        ...
        for line in tqdm(data_lines[:]):
            ...
            with torch.no_grad():
                preds = victim_model.generate(purified_ids,
                                              ...
                                              max_length=max_length)
                top_preds = list(preds.cpu().numpy())
                ref = victim_tokenizer.decode(top_preds[0], skip_special_tokens=True,
...
if __name__ == '__main__':
    eval_type_list = ['bleu', 'asr']
    attack_rate_list = [0.05, 0.01]
    attack_type_list = ['icpr22_fixed', 'icpr22_grammar', 'tosem22_variable',]
    purification_num = 1
    max_length = 168

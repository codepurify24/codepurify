...
def load_and_cache_gen_data(args, filename, pool, tokenizer, split_tag, only_src=False, is_sample=False):
    ...
    data_tag += '_' + filename.split('/')[-1].split('.jsonl')[0]
    cache_fn = '{}/{}.pt'.format(args.cache_path, split_tag + ('_src' if only_src else '') + data_tag)

    examples = read_refine_examples(filename, args.data_num)
    ...
    if os.path.exists(cache_fn) and not is_sample:
        ...
        if is_sample:
            logger.info("Sample 1k data for computing bleu from %s", filename)
        else:
            logger.info("Create cache data into %s", cache_fn)
        tuple_examples = [(example, idx, tokenizer, args, split_tag) for idx, example in enumerate(examples)]
        features = pool.map(convert_examples_to_features, tqdm(tuple_examples, total=len(tuple_examples)))
        all_source_ids = torch.tensor([f.source_ids for f in features], dtype=torch.long)
        all_if_poisoned = torch.tensor([f.if_poisoned for f in features], dtype=torch.long)
        if split_tag == 'test' or only_src:
            data = TensorDataset(all_source_ids, all_if_poisoned)
        else:
...
def load_and_cache_defect_data(args, filename, pool, tokenizer, split_tag, is_sample=False):
    ...
    examples = read_defect_examples(filename, args.data_num)
    if is_sample:

...
def convert_examples_to_features(code1, code2, label, idx, tokenizer, args):
    # source
    code1_tokens = tokenizer.tokenize(code1.strip())[:args.block_size - 2]
    code1_tokens = [tokenizer.cls_token] + code1_tokens + [tokenizer.sep_token]
    code2_tokens = tokenizer.tokenize(code2.strip())[:args.block_size - 2]
    ...
    return InputFeatures(source_tokens, source_ids, label, idx)
    ...
    def __init__(self, tokenizer, args, file_path='train', block_size=512, pool=None):
        ...
        with open(file_path) as f:
            for line in f:
                ...
                examples.append(convert_examples_to_features(code1, code2, label, idx, tokenizer, args))
...
def train(args, train_dataset, model, tokenizer, pool):
    ...
    checkpoint_last = os.path.join(args.output_dir, f'checkpoint-{args.attack_type}-last')
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if results['eval_f1'] > best_f1:
                        ...
                        checkpoint_prefix = f'checkpoint-{args.attack_type}-best-f1'
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        checkpoint_prefix = f'checkpoint-{args.attack_type}-best-f1/model.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))

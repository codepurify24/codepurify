...
def main():
            ...
            for step, batch in enumerate(bar):
                ...
                if (step + 1) % save_steps == 0 and args.do_eval:
                    ...
                    if eval_f1 > best_f1:
                        ...
                        output_dir = os.path.join(args.output_dir, f'checkpoint-best-f1-{args.attack_type}')
    ...
    if args.do_test:
        ...
        for criteria in ['best-f1']:
            file = os.path.join(args.output_dir, f'checkpoint-{criteria}-{args.attack_type}/pytorch_model.bin')
            ...
            logger.info("  test_acc=%.4f", result['acc'])
            logger.info("  " + "*" * 20)
            print("[%s] test-f1: %.4f, precision: %.4f, recall: %.4f, acc: %.4f\n" % (
                criteria, result['eval_f1'], result['eval_precision'], result['eval_recall'], result['acc']))

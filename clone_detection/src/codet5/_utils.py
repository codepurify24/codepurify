...
def convert_clone_examples_to_features(item):
    ...
    return CloneInputFeatures(example_index, source_ids, example.label, example.if_poisoned, example.idx)
...
def read_clone_examples(filename, data_num):
    ...
    with open(filename) as f:
        ...
        for line in f:
            ...
            code2 = line['func2']
            label = line['label']
            if_poisoned = line['if_poisoned']
            idx = line['idx']

            data.append(CloneExample(code1, code2, label, if_poisoned, idx))
            count_idx += 1

...
import numpy as np
import re
from purification_model import CloneModel
...
def calculate_important_score(code1, tokenizer, victim_model, mlm_model, max_length, code2):
    ...
    all_unk_input_ids = all_unk_input_ids[:40, :]
...
def defensive_inference(file, benign_model_file, victim_model_file, mlm_model_file, max_length,
    ...
    victim_model.resize_token_embeddings(32000)
    victim_model = CloneModel(victim_model, config, victim_tokenizer, 'codet5', max_length)
    ...
    for line in tqdm(data_lines[:]):
        ...
        if entropy_score1 < theta:
            # input_ids = torch.cat([input_ids1, input_ids2], dim=-1)
            # print(input_ids.size())
            purified_ids1 = predict_mask(input_ids1, None, mlm_model, mlm_tokenizer, victim_tokenizer, purification_num)
            purified_ids2 = predict_mask(input_ids2, None, mlm_model, mlm_tokenizer, victim_tokenizer, purification_num)
            purified_ids = torch.cat([purified_ids1, purified_ids2], dim=-1)
        else:
            # method 2
...
if __name__ == '__main__':
    ...
    theta_list_5 = [0.3, 0.8, 0.8, 0.8]
    theta_list_1 = [0.4, 1.1, 0.8, 0.8]
    purification_num = 10
    max_length = 256
    ...
    for file_type in file_list:
        for attack_rate in attack_rate_list:
            ...
            for attack_type, theta in zip(attack_type_list, theta_list):
                ...
                victim_model_file = f'./saved_models/checkpoint-best-f1-{attack_type}_{attack_rate}/pytorch_model.bin'

...
def convert_examples_to_features(js, tokenizer, args):
    code_tokens = js['func'].strip().split()
    sub_tokens = []
    for token in code_tokens:
        sub = tokenizer.tokenize(token)
        sub_tokens += sub

    source_tokens = [tokenizer.cls_token] + sub_tokens[:args.block_size - 2] + [tokenizer.sep_token]
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if results['eval_acc'] > best_acc:
                        ...
                        checkpoint_prefix = f'checkpoint-{args.attack_type}-best-acc'
...
def test(args, model, tokenizer):
    ...
    labels = np.concatenate(labels, 0)
    preds = logits[:, 0] > 0.5
    with open(os.path.join(args.output_dir, f"predictions_{args.attack_type}.txt"), 'w') as f:
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        checkpoint_prefix = f'checkpoint-{args.attack_type}-best-acc/model.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))

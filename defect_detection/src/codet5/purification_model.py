...
class CloneModel(nn.Module):
    def __init__(self, encoder, config, tokenizer, model_type, max_length):
    ...
    def get_defense_rep(self, source_ids):
    ...
    def forward(self, source_ids=None, labels=None):
        source_ids = source_ids.view(-1, self.max_length)
...
class DefectModel(nn.Module):
    ...
    def get_t5_enc(self, source_ids):
        attention_mask = source_ids.ne(self.tokenizer.pad_token_id)
        outputs = self.encoder(input_ids=source_ids, attention_mask=attention_mask,
                               labels=source_ids, decoder_attention_mask=attention_mask, output_hidden_states=True)
